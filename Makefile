...
distribution :
	$(LHA) ao5 $(DISTRIBUTION) $(SONNETLIB_N)/$(SONNETLIB_LIB) $(SONNETLIB_N)/$(POWERLIB_LIB) $(TOOLS_N)/$(TOOLS_GETINFO) $(TOOLS_N)/$(TOOLS_GETINFOPPC) $(TOOLS_N)/$(TOOLS_INITSONNET) $(TOOLS_N)/$(TOOLS_INITPPC) $(SONNETRUN_N)/$(SONNETRUN_SONNETPATCH) $(SONNETO_DIR)/$(SONNETO_O) $(SONNETO_DIR)/$(SONNETO_X) $(WOSDB_N)/$(WOSDB_WOSDB) README.md
