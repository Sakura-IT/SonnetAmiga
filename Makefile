...
distribution :
	$(LHA) ao5 $(DISTRIBUTION) $(SONNETLIB_N)/$(POWERLIB_LIB) $(TOOLS_N)/$(TOOLS_GETINFO) $(TOOLS_N)/$(TOOLS_GETINFOPPC) $(TOOLS_N)/$(TOOLS_INITPPC) $(WOSDB_N)/$(WOSDB_WOSDB) $(VDA_N)/$(VDA_VDAPPC) $(TOOLS_N)/$(TOOLS_BOGOMIPS) $(TOOLS_FX)/$(TOOLS_GETTEMP) $(TOOLS_FX)/$(TOOLS_GETSPEED) $(TOOLS_FX)/$(TOOLS_SETSPEEDD) README.md

.PHONY: $(SONNETLIB_N) $(SONNETRUN_N) $(TOOLS_N) $(WOSDB_N) $(VDA_N)
