...
all :  $(SONNETLIB_N) $(TOOLS_N) $(WOSDB_N) $(VDA_N)
...
distribution :
	$(LHA) ao5 $(DISTRIBUTION) $(SONNETLIB_N)/$(POWERLIB_LIB) $(TOOLS_N)/$(TOOLS_GETINFO) $(TOOLS_N)/$(TOOLS_GETINFOPPC) $(TOOLS_N)/$(TOOLS_INITPPC) $(WOSDB_N)/$(WOSDB_WOSDB) $(VDA_N)/$(VDA_VDAPPC) $(TOOLS_N)/$(TOOLS_BOGOMIPS) README.md
